...
class TestMotorOverheat:
    ...
    config = None  # Class variable to store config
    ...
    def setup(self, config):
        ...
        """
        TestMotorOverheat.config = config
    ...
    def verify_alarm_received_in_time(self, mqtt_ops, trigger_time, operation_name="MotorOverheat"):
        ...
        """
        alarm_received = mqtt_ops.check_operation(operation_name)
        
        response_time = mqtt_ops.get_alarm_response_time(trigger_time)
        if alarm_received:
            if response_time > self.config['test']['alarm_receive_timeout'] and response_time < self.config['test']['wait_for_alarm']:
                mqtt_ops.reset_operation(operation_name, True, f"Alarm received in {response_time:.3f}s")
                assert True, f"Alarm received as expected (response time: {response_time:.3f}s)"
            else:
                mqtt_ops.reset_operation(operation_name, False, f"Alarm received in {response_time:.3f}s outside expected range")
                pytest.fail(f"Alarm received but outside expected time range: {response_time:.3f}s")
        else:
            mqtt_ops.reset_operation(operation_name, False, "Alarm not received")
            pytest.fail("Alarm NOT received")
    
    def verify_no_alarm_received(self, mqtt_ops, trigger_time, operation_name="MotorOverheat"):
        ...
        if alarm_received:
            ...
            mqtt_ops.reset_operation(operation_name, False, f"Unexpected alarm received in {response_time:.3f}s")
